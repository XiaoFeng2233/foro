...
const doFollow = async (item) => {
  const response = await followUser(item.followedUserId)
...
const doUnFollow = async (item) => {
  const response = await unFollowUser(item.followedUserId)
  if(response.code === 200){
    ElMessage({
      message:response.data,
      type:"success",
    ...
    <div class="card">
      <div class="card-body p-4 text-center">
                <span class="avatar avatar-xl mb-3 avatar-rounded"
                      :style="{backgroundImage: `url(${item.followedUser.avatar})`}"></span>
        <h3 class="m-0 mb-1">
          <router-link :to="{name:'OtherUserCenterPanel',params:{id:item.followedUserId}}" class="text-reset">
            {{ item.followedUser.nickname }}
          </router-link>
        </h3>
      ...
      <div class="d-flex">
        ...
        <router-link :to="{name:'OtherUserCenterPanel',params:{id:item.followedUserId}}" class="card-btn">
