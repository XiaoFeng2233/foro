              ...
              <div class="col-md-12">
                <div class="d-flex align-items-center">
                  <div class="text-muted-sm" style="margin-top: 5px">
                    ...
                    <router-link :to="{name:'OtherUserCenterPanel',params:{id:item.user.id}}" class="text-muted-sm">
                      {{ item.user.nickname }}
                    </router-link>
...
import {queryHotTopicByTagId, queryLatestTopicByTagId, queryRecommendTopicByTagId} from "@/api/topic.js";
...
const queryTopic = async () => {
  ...
  if (sort.value === "latest") {
    response = await queryLatestTopicByTagId(pageNum.value, pageSize,route.params.id)
  }
  if (sort.value === "hot") {
    response = await queryHotTopicByTagId(pageNum.value, pageSize,route.params.id)
  }
  if (sort.value === "recommend") {
    response = await queryRecommendTopicByTagId(pageNum.value, pageSize,route.params.id)
  }
